export function createCourierModule(ctx) {
  ...
  async function renderReady() {
    page().innerHTML = `<section class="hero compact"><div><div class="eyebrow">SIAP DIAMBIL</div><h1>Pilih Rute yang Sejalan</h1><p>Cocokkan nama pasien, kode paket, dan wilayah sebelum mengambil tugas.</p></div><div class="hero-actions"><button id="readyRefresh" class="secondary-btn">↻ Segarkan</button></div></section>
      <section class="section"><div class="toolbar-card"><div class="search-box"><span>⌕</span><input id="readySearch" placeholder="Cari kelurahan, kecamatan, kota…" value="${esc(state.search)}"></div><div class="privacy-chip">Data minimum • nama + kode paket + wilayah</div></div><div id="readyContent"><div class="inline-loading">Memuat antrean…</div></div></section>`;
  ...
  function renderReadyData() {
    ...
    box.innerHTML = `<div class="route-groups">${groups.map(g => `<section class="route-group"><div class="route-group-head"><div><div class="eyebrow">${esc(g.district || g.region || 'WILAYAH')}</div><h3>${esc(g.village || '-')}</h3><p>${esc(g.region || '')}</p></div><span>${g.items.length} paket</span></div><div class="ready-card-grid">${g.items.map(r => `<article class="ready-route-card"><div class="ready-route-top"><strong class="package-code">${esc(r.packageCode || shortId(r.id))}</strong><div>${r.isRetry?`<span class="retry-chip">PENGANTARAN ULANG • Attempt ${Number(r.attemptNo||2)}</span>`:''}</div></div><h4 class="ready-patient-name">${esc(r.name || '-')}</h4><div class="ready-location-block"><span>LOKASI</span><h4>${esc(r.village || '-')}</h4><p>${esc([r.district,r.region].filter(Boolean).join(' • ') || '-')}</p><small>Siap ${esc(timeOnly(r.readyAt))}${r.coverageStatus ? ` • ${esc(r.coverageStatus)}` : ''}</small></div>${r.courierNote?`<div class="courier-note-preview"><span>CATATAN FARMASI</span><p>${esc(r.courierNote)}</p></div>`:''}<button class="primary-btn ready-claim-btn" data-claim="${esc(r.id)}">Ambil Tugas</button></article>`).join('')}</div></section>`).join('')}</div>`;
