import { APP_CONFIG } from './config.js?v=6.1.8-hf5b';
...
export function createFarmasiModule(ctx) {
  ...
  function renderTodayRows() {
    ...
    box.innerHTML = `<div class="table-wrap"><table class="responsive-table"><thead><tr><th>Jam</th><th>No. RM / Pasien</th><th>Wilayah</th><th>Status</th><th>Verifikasi</th><th>Aksi</th></tr></thead><tbody>${state.rows.map(r => `<tr>
      ...
      <td data-label="Pasien"><strong>${esc(r['Nama Pasien'])}</strong>${r.__packageCode?`<span class="package-chip">${esc(r.__packageCode)}</span>`:''}${r.__isRetry?`<span class="retry-chip farmasi-retry-chip">PENGANTARAN ULANG • Attempt ${Number(r.__attemptNo||2)}</span>`:''}<span class="cell-sub">No. RM ${esc(r['No RM'])}</span>${r.__courierNote?`<span class="cell-note"><b>Catatan Kurir:</b> ${esc(r.__courierNote)}</span>`:''}</td>
  ...
  function renderVerificationRows() {
    ...
    box.innerHTML = `<div class="table-wrap"><table class="responsive-table"><thead><tr><th>Waktu Terkirim</th><th>Pasien</th><th>Kurir</th><th>Penerima Aktual</th><th>Alasan Tanpa Kode</th><th>Aksi</th></tr></thead><tbody>${state.pending.map(r => `<tr>
      ...
      <td data-label="Pasien"><strong>${esc(r['Nama Pasien'])}</strong>${r.__isRetry?`<span class="retry-chip farmasi-retry-chip">PENGANTARAN ULANG • Attempt ${Number(r.__attemptNo||2)}</span>`:''}<span class="cell-sub">No. RM ${esc(r['No RM'])}</span>${r.__courierNote?`<span class="cell-note"><b>Catatan Kurir:</b> ${esc(r.__courierNote)}</span>`:''}</td>
