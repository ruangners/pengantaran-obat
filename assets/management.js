export function createManagementModule(ctx) {
  ...
  function timeInsight(value, label, description = '') {
    ...
    if (sample < MIN_STATS_SAMPLE) {
      const main = sample === 1 ? Number(x.average || x.median || 0) : Number(x.average || 0);
      return `<article class="time-insight"><span>${esc(label)}</span><strong>${minutes(main)}</strong><small>${sample} pengantaran dianalisis. Data belum cukup untuk analisis pola waktu.</small></article>`;
    }
    return `<article class="time-insight"><span>${esc(label)}</span><div class="time-insight-grid"><div><small>Nilai tengah</small><strong>${minutes(x.median)}</strong></div><div><small>90% selesai dalam</small><strong>≤ ${minutes(x.p90)}</strong></div></div><small>${n(sample)} pengantaran dianalisis${description ? ` • ${esc(description)}` : ''}.</small></article>`;
  }
  ...
  function performanceDurationCell(stats = {}) {
    ...
    if (!sample) return '—';
    if (sample < MIN_STATS_SAMPLE) return `<strong>${minutes(stats.average)}</strong><small class="table-subtext">${n(sample)} pengantaran</small>`;
    return `<strong>${minutes(stats.median)}</strong><small class="table-subtext">90% ≤ ${minutes(stats.p90)}</small>`;
  }
