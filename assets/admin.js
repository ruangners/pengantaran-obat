export function createAdminModule(ctx) {
  ...
  function drawArchive() {
    ...
    const fidelityCard = `<div class="content-card fidelity-health-card ${fidelityChecked&&fidelityIssues.length?'warning-card':''}"><div class="card-head-row"><div><span class="eyebrow">INTEGRITAS FORMAT DATA</span><h3>${!fidelityChecked?'Periksa tipe data dan validasi penting seluruh sistem':fidelityIssues.length?`⚠ ${fidelityIssues.length} sheet perlu diperbaiki`:'✓ Tipe data kritis terdeteksi sehat'}</h3><p>${!fidelityChecked?'Pemeriksaan berfokus pada tipe nilai sebenarnya. Perbedaan gaya tampilan tanggal yang tetap terbaca sebagai Date tidak dianggap kerusakan.':fidelityIssues.length?'Ada nilai yang masih tersedia tetapi tipe internalnya tidak lagi sesuai schema aplikasi.':'Tanggal/waktu, PIN, dan validasi kritis yang diperiksa sesuai schema aplikasi.'}</p></div><button id="checkDataFidelity" class="secondary-btn">${fidelityChecked?'Periksa Lagi':'Periksa Format Data'}</button></div>${fidelityChecked?`<div class="protection-summary"><div><span>Status</span><strong>${esc(fidelity.status||'-')}</strong></div><div><span>Sheet diperiksa</span><strong>${fidelityRows.length}</strong></div><div><span>Perlu perbaikan</span><strong>${fidelityIssues.length}</strong></div></div>${fidelityIssues.length?`<div class="structure-issue-list">${fidelityIssues.map(item=>{const examples=(item.samples||[]).slice(0,3);return `<div class="structure-issue"><div><strong>${esc(systemSheetLabel(item.name))}</strong><span>${esc(item.name)} • tipe tanggal/waktu: ${Number(item.dateIssues||0)} • PIN: ${Number(item.pinIssues||0)} • validasi: ${Number(item.validationIssues||0)} • acuan: schema aplikasi${item.source?.name?` • template validasi: ${esc(item.source.name)}`:''}</span>${examples.length?`<span>Contoh: ${examples.map(x=>`${esc(x.cell)} ${esc(x.field)} = ${esc(x.display||'(kosong)')} [${esc(x.valueType||'-')}]`).join(' • ')}</span>`:''}</div></div>`;}).join('')}</div><div class="fidelity-actions"><button id="repairDataFidelity" class="warning-btn">Perbaiki Format Data</button></div>`:`<div class="system-alert success compact-alert"><strong>Tidak ditemukan kerusakan tipe data kritis.</strong><span>Perbedaan format tampilan tanggal yang tetap memiliki tipe Date dianggap sehat.</span></div>`}${lastFidelityRepair?`<div class="fidelity-last">Perbaikan terakhir: <strong>${fmtDateTime(lastFidelityRepair.at)}</strong> • ${lastFidelityRepair.ok===false?'masih perlu pemeriksaan':'selesai'}</div>`:''}`:`<div class="system-alert info compact-alert"><strong>Pemeriksaan manual saat diperlukan</strong><span>Jalankan setelah proses pemulihan besar atau bila tanggal/waktu di Spreadsheet terlihat berubah menjadi angka serial atau teks.</span></div>`}</div>`;
  ...
  async function repairDataFidelity_(triggerButton=null) {
    const pin=await requestAdminPin_({title:'Perbaiki integritas format data?',message:'Sistem akan membuat backup pengaman lalu memperbaiki hanya nilai yang tipe internalnya terdeteksi bermasalah. Tanggal yang sudah bertipe Date tidak diubah hanya karena gaya tampilannya berbeda.',confirmLabel:'Ya, Perbaiki Format'});
    if(!pin)return;
